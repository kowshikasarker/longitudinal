...
def train_sample(sample, hidden_dim, lr, add_self_loops, model_name, out_dir):
    ...
    for epoch in range(1, max_epoch+1):
        ...
        pred = torch.squeeze(out[sample.met_mask, :, :])
...
def construct_network(met_path, met_to_react_path, react_to_sub_path):
    ...
    print(len(set(met_to_react['reaction_id'])), 'reactions')
    ...
    print(len(met_nodes), 'met_nodes', len(react_nodes), 'react_nodes', len(sub_nodes), 'sub_nodes', len(nodes), 'nodes')
